...
def main(ind,k_i,trees,boolean,crit):
	_file = open('rndmforest_'+str(ind)+'.txt','w')
	...
	for k, (train, test) in enumerate(kf.split(features, label)):
		...
		# n_estimators = 8, 16, 64, 128 : bootstrap = True, False : criterion = gini, entropy
		clf = RandomForestClassifier(n_estimators = trees, bootstrap=boolean, criterion=crit)
		#clf = RandomForestClassifier(n_estimators = 8, bootstrap=True, criterion='gini')
		features_train=features[train].astype('int')
		label_train = label[train].astype('int')
		features_test=features[test].astype('int')
		label_test = label[test].astype('int')
		clf.fit(features_train, label_train)

		# New stuff
		estimator = clf.estimators_[5]
		export_graphviz(estimator, out_file='tree_nonlimited.dot', feature_names = features, class_names = label, rounded = True, proportion = False, precision = 2, filled = True)
	...
	_file.write("------:K-fold: " + str(k_i) + " :trees: " + str(trees) + " :bootstrap: " + str(boolean)+ " :criterion: " + crit  + '----------\n')
	_file.close()

# Best result
simulation = []
for nTrees in [96, 64, 80]:
    for k in [3, 5]:
        for isBootstrap in[True, False]:
            for idkWhat in ['gini', 'entropy']:
                simulation.append((k, nTrees, isBootstrap, idkWhat))
ind=1
for entry in simulation:
	main(ind,entry[0],entry[1],entry[2],entry[3])
	ind+=1
#gives importance of features
#print sorted(list(zip(features[feature_text], clf.feature_importances_)),key = lambda x: x[1])
