...
def enhancedFeatureExtractorDigit(datum):
    ...
    I have have calculated the number of distinct areas in the datum.
    An area is a part of the datum that is all surrounded by black/grey pixels.
    If there is only one area, it means that the digit is 1, 2, 3, 5, or 7.
    If there are two, it means that the digit is 0, 6, or 9.
    If there are three, it means that the digit is 8.
    The digit '4' can either have 2 or 3 areas, depending on how it's written.
    ##
    """
    features = basicFeatureExtractorDigit(datum)
    areas = 0

    unvisited = [(x, y) for (x, y) in features.keys() if features[(x, y)] == 0]
    while len(unvisited) > 0:
        visit(unvisited)
        areas += 1

    features['oneArea'] = int(areas == 1)     # 1, 2, 3, 4, 5, 7
    features['twoAreas'] = int(areas == 2)    # 0, 4, 6, 9
    features['threeAreas'] = int(areas == 3)  # 8
...
def enhancedPacmanFeatures(state, action):
    ...
    isFoodPresent = state.getFood()
    foods = []
    for x in range(isFoodPresent.width):
        for y in range(isFoodPresent.height):
            if isFoodPresent[x][y]:
                foods.append((x, y))
    features["ClosestFood"] = min([util.manhattanDistance(pacman, food)
                                   for food in foods])

    ghosts = successor.getGhostPositions()
    features["ClosestGhost"] = min([util.manhattanDistance(pacman, ghost)
                                    for ghost in ghosts])
